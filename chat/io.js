...
const {Messages} = require('../sequelize');
//const AuthService = require('angular-src/src/app/services/auth.service');
...
const initialize = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("message", (data) => {
      ...
      //saving message in db if chat message
      if (data.message.type === undefined) {
          Messages.create(data.message);
      }
    });
